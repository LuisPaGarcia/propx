...
function check(path, value, def) {
	var out = get(obj, path, def);
	assert.strictEqual(out, value, 'get(obj, "' + path + '") should be ' + value + ', got ' + out);
	console.log(' ✓ get(obj, "' + path + '"' + (def ? ', "' + def + '"' : '') + ')');

	if (path) {
		var arr = path.split('.');
		assert.strictEqual(get(obj, arr, def), value);
		console.log(' ✓ get(obj, ' + JSON.stringify(arr) + (def ? ', "' + def + '"' : '') + ')');
